...
const clubmates = computed(() => page.props.clubmates ?? []);
    ...
    <GeneralLayout>
        ...
        <div class="py-12">
            <div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
                ...
                <div v-if="currentClub && clubmates.length > 0" class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="border-b border-gray-200 px-6 py-4">
                        <div class="flex items-center justify-between">
                            <div>
                                <h2 class="text-lg font-semibold text-gray-900">Socorristas del club {{ currentClub.name }}</h2>
                                <p class="text-sm text-gray-600">{{ clubmates.length }} miembro(s).</p>
                            </div>
                        </div>
                        <div class="p-6">
                            <SocorristasClub :clubmates="clubmates" />
                        </div>
                    </div>
                </div>

                <div v-if="currentClub && !clubmates.length" class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
